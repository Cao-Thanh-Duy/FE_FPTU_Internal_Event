...
const AdminDashboardPage = () => {
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            const [usersRes, eventsRes, venuesRes, speakersRes, slotsRes] = await Promise.all([
            ...
            const approvedEvents = events.filter(e => e.status === 'Approve' || e.status === 'Approved').length;
            const rejectedEvents = events.filter(e => e.status === 'Reject' || e.status === 'Rejected').length;
                    ...
                    <div className="stats-grid">
                        ...
                        <div className="stat-card stat-card-purple">
                            ...
                            <div className="stat-details">
                                ...
                                <p className="stat-description">Total tickets sold across all events</p>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        {dashboardData.recentEvents.map((event) => (
                                            <tr key={event.eventId}>
                                                ...
                                                <td>{new Date(event.eventDay).toLocaleDateString('vi-VN')}</td>
                                                <td>{event. venueName || 'N/A'}</td>
                                                <td>
                                                    <span className="ticket-count">
                                                        {event. maxTickerCount - (event. currentTickerCount || 0)} / {event.maxTickerCount}
                                                    </span>
                                                </td>
                                                <td>
